...
struct thread_args {
	orbit_module *ob;
	TTASEventMutex *mutex;
	std::atomic_uint64_t *counter;
};

#define errno_assert(str, cond) \
	do { \
		if (!(cond)) { \
			int err = errno; \
			std::cerr << str " returns " << ret << " : " \
				<< strerror(err) << std::endl; \
			abort(); \
		} \
	} while (0)

std::atomic_uint64_t generations;

void *thread_func(void *aux) {
	thread_args *args = (thread_args*)aux;
...
int main(int argc, const char *argv[]) {
	...
	for (int i = 0; i < NTHD; ++i) {
		ret = pthread_create(thds + i, NULL, thread_func, &args);
	}

	int sleep_time = 1;

	while (true) {
		uint64_t oldgen = generations;
		sleep(sleep_time);
		uint64_t newgen = generations;

		std::cerr << "Current generation is " << newgen << std::endl;

		if (newgen == oldgen) {
			std::cerr << "Program is stuck now!" << std::endl;
			sleep_time = 10;
		}
	}
